...
func surveyExistingDid() error {
	...
	case "Add Assertion Method":
		// Add Assertion Method
		controller := fmt.Sprintf("%s#test-%d", doc.GetID(), doc.ControllerCount())
		...
		pub := priv.PubKey()
		vm, err := did.NewVerificationMethod(doc.GetID(), ssi.ECDSASECP256K1VerificationKey2019, *didController, pub)
		cobra.CheckErr(err)
	...
	case "Add Invocation Method":
		...
		priv := secp256k1.GenPrivKey()
		pub := priv.PubKey()
		vm, _ := did.NewVerificationMethod(doc.GetID(), ssi.ECDSASECP256K1VerificationKey2019, *didController, pub)
		vm.Credential = CreateMockCredential()
