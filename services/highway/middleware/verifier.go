...
func IssueCredentialAssertionOptions(email string, record *servicetypes.ServiceRecord) (string, protocol.URLEncodedBase64, string, error) {
	...
	cont, err := didcontroller.AuthorizeIdentity(email, controllerAcc)
	if err != nil {
...
func UseControllerAccount(token string) (*didcontroller.SonrController, error) {
	ctx := context.Background()
	...
	controller, err := didcontroller.AuthorizeIdentity(claims.Email, acc)
	if err != nil {
		return nil, fmt.Errorf("failed to authorize identity: %w", err)
	}
...
func PublishControllerAccount(alias string, cred *webauthn.Credential, origin string) (*didcontroller.SonrController, *types.TxResponse, error) {
	controller, err := didcontroller.New(alias, cred, origin)
	if err != nil {
...
func CreateOrganizationRecord(name string, origin string, admin string, controller *didcontroller.SonrController) error {
	return nil
}
