...
import (
	...
	"github.com/sonr-io/sonr/x/sonr/types"
...
type HighwayStub struct {
	...
	*exchange.ExchangeProtocol
	...
	buckets  map[string]*common.Bucket
	objects  map[string]*common.ObjectDoc
}
...
func NewHighwayStub(ctx context.Context, n node.NodeImpl, loc *common.Location, lst net.Listener) (*HighwayStub, error) {
	...
	if err = highway.RegisterHighwayServiceHandlerFromEndpoint(ctx, stub.mux, "localhost:8080", opts); err != nil {
...
func (s *HighwayStub) CreateChannel(ctx context.Context, req *highway.CreateChannelRequest) (*highway.CreateChannelResponse, error) {
	// Create DID
	did, err := common.NewDID(req.Name)
	if err != nil {
		return nil, err
	}

	// Create the Channel
	ch, err := channel.New(ctx, s.node, did)
	if err != nil {
		return nil, err
	}

	// Add to the list of Channels
	s.channels[did.String()] = ch
	return nil, ErrMethodUnimplemented
}
