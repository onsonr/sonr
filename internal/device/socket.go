...
func OpenUnixSocket() (string, error) {
	if sockManager == nil {
		return "", errors.New("Socket Manager has not been initialized")
	}
	sockManager.muCounter.Lock()
	if sockManager.counter == math.MaxUint32 {
		sockManager.muCounter.Unlock()
		return "", errors.New("max number of socket exceeded")
	}
	sockFilename := fmt.Sprintf(paddingFormatStr, strconv.FormatUint(uint64(sockManager.counter), 36))
	sockManager.counter++
	sockManager.muCounter.Unlock()

	sockPath := filepath.Join(sockManager.sockDirPath, sockFilename)
...
func Reset() {
	if sockManager == nil {
		logger.Warn("Socket Manager has not been initialized")
		return
	}
	sockManager.muCounter.Lock()
	sockManager.counter = 0
	sockManager.muCounter.Unlock()
}
