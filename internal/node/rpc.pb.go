// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.27.1
// 	protoc        v3.17.3
// source: proto/client/rpc.proto

package node

import (
	common "github.com/sonr-io/core/internal/common"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type Empty struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *Empty) Reset() {
	*x = Empty{}
	if protoimpl.UnsafeEnabled {
		mi := &file_proto_client_rpc_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Empty) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Empty) ProtoMessage() {}

func (x *Empty) ProtoReflect() protoreflect.Message {
	mi := &file_proto_client_rpc_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Empty.ProtoReflect.Descriptor instead.
func (*Empty) Descriptor() ([]byte, []int) {
	return file_proto_client_rpc_proto_rawDescGZIP(), []int{0}
}

var File_proto_client_rpc_proto protoreflect.FileDescriptor

var file_proto_client_rpc_proto_rawDesc = []byte{
	,
}

var (
	file_proto_client_rpc_proto_rawDescOnce sync.Once
	file_proto_client_rpc_proto_rawDescData = file_proto_client_rpc_proto_rawDesc
)

func file_proto_client_rpc_proto_rawDescGZIP() []byte {
	file_proto_client_rpc_proto_rawDescOnce.Do(func() {
		file_proto_client_rpc_proto_rawDescData = protoimpl.X.CompressGZIP(file_proto_client_rpc_proto_rawDescData)
	})
	return file_proto_client_rpc_proto_rawDescData
}

var file_proto_client_rpc_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_proto_client_rpc_proto_goTypes = []interface{}{
	(*Empty)(nil),                // 0: sonr.node.Empty
	(*SupplyRequest)(nil),        // 1: sonr.node.SupplyRequest
	(*EditRequest)(nil),          // 2: sonr.node.EditRequest
	(*ShareRequest)(nil),         // 3: sonr.node.ShareRequest
	(*RespondRequest)(nil),       // 4: sonr.node.RespondRequest
	(*SearchRequest)(nil),        // 5: sonr.node.SearchRequest
	(*SupplyResponse)(nil),       // 6: sonr.node.SupplyResponse
	(*EditResponse)(nil),         // 7: sonr.node.EditResponse
	(*ShareResponse)(nil),        // 8: sonr.node.ShareResponse
	(*RespondResponse)(nil),      // 9: sonr.node.RespondResponse
	(*SearchResponse)(nil),       // 10: sonr.node.SearchResponse
	(*common.StatusEvent)(nil),   // 11: sonr.core.StatusEvent
	(*common.DecisionEvent)(nil), // 12: sonr.core.DecisionEvent
	(*common.InviteEvent)(nil),   // 13: sonr.core.InviteEvent
	(*common.ProgressEvent)(nil), // 14: sonr.core.ProgressEvent
	(*common.CompleteEvent)(nil), // 15: sonr.core.CompleteEvent
}
var file_proto_client_rpc_proto_depIdxs = []int32{
	1,  // 0: sonr.node.NodeService.Supply:input_type -> sonr.node.SupplyRequest
	2,  // 1: sonr.node.NodeService.Edit:input_type -> sonr.node.EditRequest
	3,  // 2: sonr.node.NodeService.Share:input_type -> sonr.node.ShareRequest
	4,  // 3: sonr.node.NodeService.Respond:input_type -> sonr.node.RespondRequest
	5,  // 4: sonr.node.NodeService.Search:input_type -> sonr.node.SearchRequest
	0,  // 5: sonr.node.NodeService.OnStatus:input_type -> sonr.node.Empty
	0,  // 6: sonr.node.NodeService.OnDecision:input_type -> sonr.node.Empty
	0,  // 7: sonr.node.NodeService.OnInvite:input_type -> sonr.node.Empty
	0,  // 8: sonr.node.NodeService.OnProgress:input_type -> sonr.node.Empty
	0,  // 9: sonr.node.NodeService.OnComplete:input_type -> sonr.node.Empty
	6,  // 10: sonr.node.NodeService.Supply:output_type -> sonr.node.SupplyResponse
	7,  // 11: sonr.node.NodeService.Edit:output_type -> sonr.node.EditResponse
	8,  // 12: sonr.node.NodeService.Share:output_type -> sonr.node.ShareResponse
	9,  // 13: sonr.node.NodeService.Respond:output_type -> sonr.node.RespondResponse
	10, // 14: sonr.node.NodeService.Search:output_type -> sonr.node.SearchResponse
	11, // 15: sonr.node.NodeService.OnStatus:output_type -> sonr.core.StatusEvent
	12, // 16: sonr.node.NodeService.OnDecision:output_type -> sonr.core.DecisionEvent
	13, // 17: sonr.node.NodeService.OnInvite:output_type -> sonr.core.InviteEvent
	14, // 18: sonr.node.NodeService.OnProgress:output_type -> sonr.core.ProgressEvent
	15, // 19: sonr.node.NodeService.OnComplete:output_type -> sonr.core.CompleteEvent
	10, // [10:20] is the sub-list for method output_type
	0,  // [0:10] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_proto_client_rpc_proto_init() }
func file_proto_client_rpc_proto_init() {
	if File_proto_client_rpc_proto != nil {
		return
	}
	file_proto_client_api_proto_init()
	if !protoimpl.UnsafeEnabled {
		file_proto_client_rpc_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Empty); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_proto_client_rpc_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_proto_client_rpc_proto_goTypes,
		DependencyIndexes: file_proto_client_rpc_proto_depIdxs,
		MessageInfos:      file_proto_client_rpc_proto_msgTypes,
	}.Build()
	File_proto_client_rpc_proto = out.File
	file_proto_client_rpc_proto_rawDesc = nil
	file_proto_client_rpc_proto_goTypes = nil
	file_proto_client_rpc_proto_depIdxs = nil
}
