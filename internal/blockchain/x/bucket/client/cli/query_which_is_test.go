...
import (
	...
	"github.com/sonr-io/sonr/internal/blockchain/x/bucket/client/cli"
...
func networkWithWhichIsObjects(t *testing.T, n int) (*network.Network, []types.WhichIs) {
	t.Helper()
	...
	return network.New(t, cfg), state.WhichIsList
}

func TestShowWhichIs(t *testing.T) {
	net, objs := networkWithWhichIsObjects(t, 2)
...
func TestListWhichIs(t *testing.T) {
	net, objs := networkWithWhichIsObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListWhichIs(), args)
			require.NoError(t, err)
			var resp types.QueryAllWhichIsResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
