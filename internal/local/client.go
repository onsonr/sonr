...
func (c LocalContext) GetService(ctx context.Context, origin string) (*identitytypes.Service, error) {
	conn, err := grpc.Dial(c.GrpcEndpoint(), grpc.WithInsecure())
	if err != nil {
		return nil, errors.New("failed to connect to grpc server: " + err.Error())
	}
...
func (c LocalContext) GetAllServices(ctx context.Context) ([]*identitytypes.Service, error) {
	conn, err := grpc.Dial(c.GrpcEndpoint(), grpc.WithInsecure())
	if err != nil {
...
// CreateService sends a transaction to create a new service
func (c LocalContext) RegisterService(service *identitytypes.Service, acc models.Account) (*BroadcastTxResponse, error) {
	msg := identitytypes.NewMsgRegisterService(acc.Address(), service)
	bz, err := cosmos.SignAnyTransactions(acc, msg)
	if err != nil {
		return nil, err
	}
