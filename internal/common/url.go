...
// ** ─── URLLink MANAGEMENT ────────────────────────────────────────────────────────
// NewURLLink Creates New Link
func NewURLLink(url string) *UrlItem {
	link := &UrlItem{
		Url: url,
	}
	link.SetData()
...
// SetData Sets URLLink Data
func (u *UrlItem) SetData() {
	// Create Request
...
func (d *Document) cleanConditionally(s *goquery.Selection, selector string) {
	...
	s.Find(selector).Each(func(i int, s *goquery.Selection) {
		...
		if strings.Count(text, ",") < 10 {
			...
			if remove {
				logger.Info(fmt.Sprintf("Conditionally cleaned %s%s with weight %f and content score %f because it has %s\n", node.Data, getName(s), weight, contentScore, reason))
				removeNodes(s)
			}
		}
	})
