...
func NewHost(ctx context.Context, dir *md.Directories, olc string, connectivity md.ConnectionRequest_Connectivity) (host.Host, error) {
	...
	if connectivity == md.ConnectionRequest_Wifi {
		...
		err = startRendevouz(ctx, h, point)
	}
	return h, err
}
