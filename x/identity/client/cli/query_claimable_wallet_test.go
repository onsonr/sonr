...
func TestListClaimableWallet(t *testing.T) {
	net, objs := networkWithClaimableWalletObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListClaimableWallet(), args)
			require.NoError(t, err)
			var resp types.QueryAllClaimableWalletResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
