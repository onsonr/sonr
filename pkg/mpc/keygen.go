...
import (
	...
	"github.com/sonrhq/core/pkg/mpc/algorithm"
...
// GetDIDAddress from the given configuration.
func GetDIDAddress(conf *cmp.Config, coinType crypto.CoinType) (string, error) {
	skPP, ok := conf.PublicPoint().(*curve.Secp256k1Point)
	if !ok {
		return "", fmt.Errorf("invalid public point type")
	}
	bz, err := skPP.MarshalBinary()
	if err != nil {
		return "", err
	}
	pub := crypto.NewSecp256k1PubKey(bz)
	addr := coinType.FormatAddress(pub)
	return fmt.Sprintf("did:%s:%s", coinType.DidMethod(), addr), nil
}
