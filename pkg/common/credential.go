...
type CredentialCreationResponse struct {
	PublicKeyCredential
	AttestationResponse protocol.AuthenticatorAttestationResponse `json:"response"`
...
func (pcc *ParsedCredentialCreationData) Verify(storedChallenge string, verifyUser bool, relyingPartyID string, relyingPartyOrigins []string) error {
	...
	verifyError := pcc.Response.CollectedClientData.Verify(storedChallenge, protocol.CreateCeremony, relyingPartyOrigins)
	if verifyError != nil {
		return verifyError
	}
...
func (ppkc ParsedPublicKeyCredential) GetAppID(authExt protocol.AuthenticationExtensions, credentialAttestationType string) (appID string, err error) {
	var (
	...
	if enableAppID, ok = clientValue.(bool); !ok {
		return "", protocol.ErrBadRequest.WithDetails("Client Output appid did not have the expected type")
	}
	...
	if value, ok = authExt[ExtensionAppID]; !ok {
		return "", protocol.ErrBadRequest.WithDetails("Session Data does not have an appid but Client Output indicates it should be set")
	}

	if appID, ok = value.(string); !ok {
		return "", protocol.ErrBadRequest.WithDetails("Session Data appid did not have the expected type")
	}
