...
const FeatureGrid = ({ features }: FeatureGridProps) => {
    return (
        <Container>
            <div className="text-primary-text md:text-md mb-16 grid w-full grid-cols-2 place-items-center gap-y-9 text-sm md:mb-[14rem] md:grid-cols-3">
        ...
        <Container>
            <div className="grid w-full grid-cols-1 gap-6 md:grid-cols-2">
                {features.map(({ title, text, image, imageClassName }) => (
                    <div
                        key={title}
                        className="before:bg-glass-gradient relative aspect-[1.1/1] overflow-hidden rounded-[2.4rem] border border-white bg-[radial-gradient(ellipse_at_center,rgba(var(--feature-color),0.05),transparent)] px-8 py-4 before:pointer-events-none before:absolute before:inset-0 md:rounded-[4.8rem] md:p-10"
                    >
